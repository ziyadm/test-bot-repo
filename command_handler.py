from airtable_client import AirtableClient
from discord_client import DiscordClient
from mission import Mission
from mission_status import MissionStatus
from question import Question
from user import User

import discord


class CommandHandler:

    def __init__(self, airtable_client: AirtableClient, discord_client: DiscordClient):
        self.airtable_client = airtable_client
        self.discord_client = discord_client

    async def get_first_unasked_question(self, user: User):
        missions = await Mission.all_for_player(airtable_client = self.airtable_client,
                                                player_discord_id = user.discord_id)
        questions_already_asked = set([mission.question_id for mission in missions])
        questions = await Question.all(airtable_client = self.airtable_client)
        for question in questions:
            if question.question_id not in questions_already_asked:
                return question


    async def new_command(self, interaction):
        user = await User.get(airtable_client = self.airtable_client,
                              discord_id = str(interaction.user.id))
        question = await self.get_first_unasked_question(user)
        
        if not question:
            return await interaction.followup.send('Monarch Suriel has no new training for you')
            
        mission_channel = await self.discord_client \
                                    .create_mission_channel(member = interaction.user,
                                                            question_id = question.question_id)

        await Mission.create(airtable_client = self.airtable_client,
                             discord_channel_id = str(mission_channel.id),
                             player_discord_id = str(interaction.user.id),
                             reviewer_discord_id = None,
                             question_id = question.question_id,
                             mission_status = MissionStatus(value = MissionStatus.design),
                             design = None,
                             code = None)

        # CR hmir: maybe send instructions to user?
        await mission_channel.send(f"""Here's your question: {question.leetcode_url}""")
        
        return await interaction.followup.send(f"""Monarch Suriel has invited you to {mission_channel.mention}""")

    async def submit_command(self, interaction):
        # CR hmir: only allow submit in mission channel
        # CR hmir: we probably wanna rename submit to fit the "mission"/"quest" theme
        mission = await Mission.get(airtable_client = self.airtable_client,
                                    discord_channel_id = str(interaction.channel_id))
    
        if not (mission.mission_status == MissionStatus.design or
                mission.mission_status == MissionStatus.code):
                    return await interaction.followup.send("""You've completed your objective, wait for Monarch Suriel's instructions!""")
    
        channel = interaction.channel
        
        messages = [message async for message in channel.history() if message.type == discord.MessageType.default]
        if len(messages) == 0:
            # CR hmir: maybe we can frame missions as you having "minions" who you have to give instructions to to solve some problems you come across in your journey?
            # alternatively it could be framed as being a world of machines and you need to make the machines do what you want. i think theres a pretty popular recent game thats similar to this
            # follow up with ziyad
            return await interaction.followup.send('You need to instruct your minions!')
    
        if mission.mission_status == MissionStatus.design:
            await Mission.update(airtable_client = self.airtable_client,
                                 record_id = mission.record_id,
                                 design = messages[0].content,
                                 mission_status = MissionStatus(value = MissionStatus.design_review))
            return await interaction.followup.send("""Planning is half the battle! We've sent your plan to Monarch Suriel for approval. Check back in about 30 minutes to find out your next objective.""")
        elif mission.mission_status == MissionStatus.code:
            await Mission.update(airtable_client = self.airtable_client,
                                 record_id = mission.record_id,
                                 code = messages[0].content,
                                 mission_status = MissionStatus(value = MissionStatus.code_review))
            return await interaction.followup.send("""Monarch Suriel will be pleased! We've sent your instructions to Him for approval. Once they're approved, they'll be sent directly to your minions on the frontlines!""")